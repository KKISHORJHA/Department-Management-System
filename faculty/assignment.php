...
{
	$msg=$_POST['assignment'];
	date_default_timezone_set("Asia/Kolkata");
	$date= date("Y-m-d G:i:s");
	$sem=$_POST['sem'];
	$select=mysqli_query($conn, "SELECT S_ID FROM student WHERE SEM='$sem'");
	...
	if($count!=0)
	{
		...
		{
			$sid=$row['S_ID'];
			$insert=mysqli_query($conn, "INSERT INTO assignment VALUES(NULL,'$sid','$msg','NOT SUBMITTED','$date')");
			if(!$insert)
			{
	...
	{
		echo '<script>alert("No Student in this Semester. please choose another Semester")</script>';
	}
